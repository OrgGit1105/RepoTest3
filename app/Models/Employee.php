...
//    'company_branch',
  ...
  {
    return $this->hasMany(RiskScore::class, 'employee_id', 'employee_code');
  }
  ...
  {
    if (is_null($this->joining_age_company)) {
      $data = $this->queryConfigRange(ConfigRange::TYPE_AGE)->whereNull('rank')->first();
      if (!$data) {
        $data = $this->queryConfigRange(ConfigRange::TYPE_AGE)->where('rank', 0)->first();
      }
    } else {
      $data = $this->queryConfigRange(ConfigRange::TYPE_AGE)->where('from', '<=', $this->joining_age_company)->where('to', '>', $this->joining_age_company)->first();
    }
    return $data;
  ...
  {
    if (is_null($this->spouse)) {
      $data =  $this->queryConfigRange(ConfigRange::TYPE_PERSON_MARRIED)->whereNull('rank')->first();
      if (!$data) {
        $data =  $this->queryConfigRange(ConfigRange::TYPE_PERSON_MARRIED)->where('rank', 0)->first();
      }
    } else {
      if ($this->spouse > 1) {
        $data =  $this->queryConfigRange(ConfigRange::TYPE_PERSON_MARRIED)->where('rank', 1)->first();
      } else {
        $data =  $this->queryConfigRange(ConfigRange::TYPE_PERSON_MARRIED)->where('rank', $this->spouse)->first();
      }
    }
    return $data;
  }
  ...
  {
    if (is_null($this->dependents)) {
      $data = $this->queryConfigRange(ConfigRange::TYPE_NUMBER_OF_DEPENDENTS)->whereNull('rank')->first();
      if (!$data) {
        $data = $this->queryConfigRange(ConfigRange::TYPE_NUMBER_OF_DEPENDENTS)->where('rank', 0)->first();
      }
    } else {
      if ($this->dependents > 3) {
        $data = $this->queryConfigRange(ConfigRange::TYPE_NUMBER_OF_DEPENDENTS)->where('rank', 3)->first();
      } else {
        $data = $this->queryConfigRange(ConfigRange::TYPE_NUMBER_OF_DEPENDENTS)->where('rank', $this->dependents)->first();
      }
    }
    return $data;
  }
  ...
  {
    if (is_null($this->worked_year)) {
      $data = $this->queryConfigRange(ConfigRange::TYPE_NUMBER_COMPANY)->whereNull('rank')->first();
      if (!$data) {
        $data = $this->queryConfigRange(ConfigRange::TYPE_NUMBER_COMPANY)->where('rank', 0)->first();
      }
    } else {
      if ($this->worked_year > 7) {
        $data = $this->queryConfigRange(ConfigRange::TYPE_NUMBER_COMPANY)->where('rank', 7)->first();
      } else {
        $data = $this->queryConfigRange(ConfigRange::TYPE_NUMBER_COMPANY)->where('rank', $this->worked_year)->first();
      }
    }
    return $data;
  }
  ...
  {
    if (is_null($this->final_education)) {
      $data = $this->queryConfigRange(ConfigRange::TYPE_CERTIFICATE)->whereNull('rank')->first();
      if (!$data) {
        $data = $this->queryConfigRange(ConfigRange::TYPE_CERTIFICATE)->where('rank', 0)->first();
      }
    } else {
      if ($this->final_education > 7) {
        $data = $this->queryConfigRange(ConfigRange::TYPE_CERTIFICATE)->where('rank', 7)->first();
      } else {
        $data = $this->queryConfigRange(ConfigRange::TYPE_CERTIFICATE)->where('rank', $this->final_education)->first();
      }
    }
  ...
  {
    if (is_null($this->shortest_service)) {
      $data =  $this->queryConfigRange(ConfigRange::TYPE_RANGE_TIME)->whereNull('rank')->first();
      if (!$data) {
        $data =  $this->queryConfigRange(ConfigRange::TYPE_RANGE_TIME)->where('rank', 0)->first();
      }
    } else {
      $data =  $this->queryConfigRange(ConfigRange::TYPE_RANGE_TIME)->where('from', '<=', $this->shortest_service)->where('to', '>', $this->shortest_service)->first();
    }
    return $data;
  }
}
